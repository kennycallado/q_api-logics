...
#[options("/<_name>")]
pub fn options_show(_name: String) -> Status {
    Status::Ok
}

#[post("/<name>/project/<id>", data = "<script>", rank = 101)]
pub async fn upload_file(claims: AccessClaims, name: String, id: i32, script: Data<'_>) -> Result<Status, Status> {
    match claims.0.user.role.name.as_str() {
        "admin" => create::save(name, id, script).await,
...
pub async fn upload_file_none(_name: String, _id: i32, _script: Data<'_>) -> Result<Status, Status> {
    Err(Status::Unauthorized)
}

#[get("/<name>/project/<id>", rank = 101)]
pub async fn show_file(claims: AccessClaims, name: String, id: i32) -> Result<rocket::tokio::fs::File, Status> {
    match claims.0.user.role.name.as_str() {
        "admin" => show::read(name, id).await,
...
pub async fn show_file_none(_name: String, _id: i32) -> Result<RawText<String>, Status> {
    Err(Status::Unauthorized)
}
